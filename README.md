...
>
> `propagate`/`rebase`/`push`/`remove` support `--dry-run` to preview first. Pass
> `--no-descendants` to `propagate`/`rebase` to act on the named branch only, without cascading.
