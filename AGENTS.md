...
- `git tree continue` (`cmd_continue`): resumes a cascade after a conflict — finishes the in-progress rebase (editor disabled), records the new fork point, propagates to descendants. Replaces raw `git rebase --continue` + `git tree propagate`. Conflict resolutions auto-replay across the cascade via rerere; `--no-auto-rerere` (on propagate/rebase/continue) disables it.
