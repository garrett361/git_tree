...
- `git tree continue <origin>` (`cmd_continue`): resumes a cascade after a conflict — finishes the in-progress rebase (editor disabled), records the new fork point, then resumes from `origin` (a **required** positional: the branch the original `propagate`/`rebase` began at) and its descendants. The scope is not stored anywhere, so the caller must name it; resuming from the tree root instead would wrongly rebase branches outside the origin's subtree (siblings/cousins carrying their own drift). A guard rejects an origin that is not the stuck branch or one of its ancestors (`code=4`, `kind=precondition`); it does not catch a mis-scoped origin (any branch between a root and the stuck branch passes): too broad over-propagates but leaves things current, too narrow silently under-propagates. Replaces raw `git rebase --continue` + `git tree propagate`. Conflict resolutions auto-replay across the cascade via rerere; `--no-auto-rerere` (on propagate/rebase/continue) disables it.
